    ...
    goMonth: for (let Mi = MIdx; Mi < MDate.length; Mi++) {
      ...
      goDay: for (let Di = DIdx; Di < DDate.length; Di++) {
        ...
        if (dayRule.value === 'lastDay') {
          ...
          const index0 = dayRuleSup.value[0]
          const index1 = dayRuleSup.value[1]
          if (dayRuleSup.value[1] >= thisWeek!) {
            DD = index0 - 1 * 7 + index1 - Number(thisWeek) + 1
          } else {
            DD = index0 * 7 + index1 - Number(thisWeek) + 1
          }
          ...
          if (dayRuleSup.value < thisWeek) {
            DD -= Number(thisWeek) - Number(dayRuleSup.value)
          } else if (dayRuleSup.value > thisWeek) {
            DD -= 7 - (Number(dayRuleSup.value) - Number(thisWeek))
          }
        }
...
function formatDate(value: Date, type?: 'week') {
  // 计算日期相关值
...
// 检查日期是否存在
function checkDate(value: number | string | Date) {
